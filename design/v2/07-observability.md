...
`shared/logging.py` 提供统一的 `AppLogger` 封装：
...
class AppLogger:
    ...
    # context 自动注入：request_id, timestamp, service_name
```
...
- **透传**：Auth Service 和 Info Service 读取并透传，所有日志自动注入 `request_id`。
- **中间件实现**：每个服务的 FastAPI middleware 读取 `X-Request-ID` Header → 存入 `ContextVar` → 日志自动提取。

### 3.2 请求日志

每个 HTTP 请求记录一条 INFO 日志：
...
- 跨服务调用失败：同时记录 `target_service`、`target_url`、`request_body`（脱敏后）、`response_status`。

## 4. 可观测性演进路线

| 阶段 | 能力 | 说明 |
|------|------|------|
| 原型 | JSON 日志 + request_id 串联 | 通过 `grep request_id` 手动排查 |
| 中期 | ELK / Loki 集中日志 | 结构化查询、自动聚合 |
| 远期 | OpenTelemetry 全链路 | Trace + Metrics + Logs 三位一体 |

> 原型阶段基础设施简单，优先保证日志结构化与 request_id 透传，确保出问题时能快速定位。
