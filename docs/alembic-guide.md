...
所有命令从**项目根目录**运行。Alembic 使用各链 `alembic.ini` 中的同步 `sqlite:///` URL，**不要**把应用/测试用的 `AUTH_DATABASE_URL`（`sqlite+aiosqlite://`）当作迁移连接串。
...
# 首次迁移前创建 SQLite 目录（Alembic 不会自动创建 data/）
mkdir -p data
...
Auth 迁移链（`auth.db`）：`001_initial_auth_schema` → `002_seed_roles_permissions_admin` → `003_auth_schema_constraints`。
...
- **迁移文件纳入版本控制**，与代码同步提交
- **首次执行迁移前**需先 `mkdir -p data`（及 `logs/`、`uploads/` 若尚未存在），否则 SQLite 会报 `unable to open database file`；应用 lifespan 会在启动时创建这些目录，但 Alembic env.py 不负责创建
- **Auth 003 与 SQLite**：`003_auth_schema_constraints` 在 SQLite 上执行 `upgrade head` 可能失败（`create_unique_constraint` 不支持）。SQLite 原型可先 `upgrade b2c3d4e5f6a7`（002，含种子）；生产 PostgreSQL 或后续将 003 改为 batch 迁移后再 `upgrade head`
- **pytest 与 Alembic 分离**：测试使用内存库 `sqlite+aiosqlite://`（见 `tests/conftest.py`），不依赖迁移文件；本地持久化开发需单独跑 Alembic
