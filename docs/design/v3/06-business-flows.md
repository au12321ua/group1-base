...
sequenceDiagram
    ...
    Info->>Auth: POST /internal/users {user_id, username, role_ids}
    Auth->>AuthDB: INSERT credentials (初始密码)
    Auth->>AuthDB: INSERT user_roles (角色分配)
...
sequenceDiagram
    ...
    loop 每条有效记录
        ...
        Info->>Auth: POST /internal/users (创建认证 + 角色)
        alt 成功
...
sequenceDiagram
    ...
    Admin->>Info: PUT /users/{id} (含 role_ids)
    Info->>Info: 更新用户业务数据
    Info->>Auth: POST /internal/users/{user_id}/roles {role_ids: [1, 2]}
    Auth->>Auth: 删除现有 user_roles
    Auth->>Auth: 插入新的 user_roles
    alt 成功
        Auth-->>Info: 200 {user_id, role_ids}
        Info-->>Admin: 200 用户更新成功
    else 失败
        Auth-->>Info: 500 角色同步失败
        Info->>Info: 记录错误日志
        Info-->>Admin: 200 用户数据已更新，但角色同步失败（降级处理）
    end
```

> **设计说明**：角色数据由 Auth Service 独立管理。Info Service 在用户更新请求中包含 `role_ids` 时，通过 `/internal/users/{user_id}/roles` 同步角色分配。角色同步失败时采用**降级策略**：用户业务数据已更新不补偿回滚，角色同步失败记录日志供后续人工处理。
...
sequenceDiagram
    ...
    Gateway->>Info: 透传 + Header {X-User-Id, X-User-Role, X-Request-ID}
    Info->>Info: 信任 Gateway Header 中的调用方身份
    Info->>Info: 查询教师主数据
    Info->>Info: 附加 snapshotTime / version
...
| P2.2 用户与档案维护 | UserManagementService | CRUD + 跨服务同步 + 角色协调 |
| P2.3 课程校历培养方案维护 | CourseManagementService | 课程/开课/排课/教室/校历/方案 CRUD |
| P2.4 回收站处理 | UserManagementService + RecycleBinService | 逻辑/物理删除 + 跨服务协调 |
