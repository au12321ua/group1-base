...
| `/courses/{id}` | DELETE | 删除课程 | `course:delete` |
...
### 2.10 Info Service — 文件

| 路径 | 方法 | 说明 | 权限 |
|------|------|------|------|
| `/files` | POST | 上传文件 | 需认证 |
| `/files/{id}` | GET | 下载文件 | 需认证 |
| `/files/{id}` | DELETE | 删除文件 | 拥有者或管理员 |

### 2.11 Info Service — 审计日志
...
### 2.12 Info Service — 数据提供（面向 B/C/F）
...
| `/data-provision/selected-students` | GET | 选课学生名单（代理至 C 系统） | B 排课 | Service Token |

> `/data-provision/*` 接口返回均含 `snapshotTime` 或 `version` 字段。`selected-students` 为代理接口，Info Service 转发请求至 C（智能选课）系统并返回结果，不在本地存储选课数据。
