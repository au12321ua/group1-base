"""AuditLog, DeadLetterQueue, OperationLog models — audit trail stored in Log DB (audit.db).

These models are shared between Auth Service and Info Service so both can write
to the same audit.db without cross-service HTTP calls.
"""
